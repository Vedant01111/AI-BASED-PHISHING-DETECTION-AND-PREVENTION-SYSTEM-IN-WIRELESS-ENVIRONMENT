...
for p in root.rglob('*'):
    ...
    elif p.is_dir() and p.name == '.playwright-mcp': shutil.rmtree(p)
    elif p.is_dir() and p.name == '.remember': shutil.rmtree(p)
    elif p.is_dir() and p.name == 'node_modules': shutil.rmtree(p)
